...
type msg struct {
	...
	tx, rx   *plan9.Fcall // transmit/receive 9P message
...
func (cfg *Config) clientIO(conn net.Conn) {
	readc := make(chan *plan9.Fcall)
	go func() {
		for {
			f, err := plan9.ReadFcall(conn)
			if err == io.EOF {
				break
			}
			if err != nil {
...
func (c *client) processRx(conn net.Conn, m *msg) {
	...
	if m.rx.Type == plan9.Ropenfd {
		if err := SendFD(conn.(*net.UnixConn), uintptr(m.rx.Unixfd)); err != nil {
